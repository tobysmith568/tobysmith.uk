...
- `bun run lint` (`eslint .`) — lint check (ESLint — see
  ...
  `eslint-plugin-playwright`'s `flat/recommended` is layered on for `e2e/**` +
  `playwright.config.ts` only (playwright-community's plugin, recommended by Playwright's own
  docs — no first-party alternative exists; `missing-playwright-await` is the notable catch).
