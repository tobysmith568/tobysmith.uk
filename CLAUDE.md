...
- `bun run lint` (`eslint .`) — lint check (ESLint — see
  `docs/adrs/0001-eslint-over-oxlint-and-biome.md` for why ESLint, not oxlint, ended up as
  the linter); use `bunx eslint . --fix` to autofix. `eslint.config.mjs` (flat config): oxlint's
  own bundled TypeScript rules are replaced by `typescript-eslint`'s `recommended` config, plus
  `eslint-plugin-astro`'s `flat/recommended` (Astro-specific correctness rules) and its
  `flat/jsx-a11y-recommended` (real `.astro` **template** accessibility linting, via
  `eslint-plugin-jsx-a11y` — this is the thing oxlint/oxfmt/Biome all fell short on; see the ADR).
  Repo-wide overrides: `@typescript-eslint/no-explicit-any` is off (matches oxlint's own
  `correctness`-only scope from before — not an opinion this repo holds), and
  `@typescript-eslint/no-unused-vars` uses `argsIgnorePattern: "^_"` (this repo's convention for
  intentionally-unused callback params). Scoped overrides: `@typescript-eslint/triple-slash-reference`
  is off for `src/env.d.ts` (Astro's own idiomatic convention for wiring up ambient types, not
  fixable via `import`), and `@typescript-eslint/no-unused-expressions` is off under `cypress/**`
  (Chai's getter-style assertions like `.to.exist` read as unused expressions to that rule).
  `dist/`, `.astro/`, and `worker-configuration.d.ts` (generated) are excluded via `ignores`.
- `bun run format:check` (`prettier --check .`) — format check for the whole repo via Prettier +
  `@tobysmith568/prettier-config` (bundles `prettier-plugin-astro`, so `.astro` and everything else
  go through the same single tool); `bun run format` (`prettier --write .`) to fix. oxfmt was
  evaluated and dropped again after adopting it — see
  `docs/adrs/0002-prettier-over-oxfmt.md`: unlike the linter decision, Prettier already formats the
  entire repo correctly on its own, so oxfmt's speed advantage wasn't backed by any capability gap
  worth the added tool. `worker-configuration.d.ts` (generated by `wrangler types`) and `dist` are
...
CI (`.github/workflows/integration.yml`) runs ESLint, Prettier, `bun run build`, and the Cypress
suite
...
- **Header/nav** (`BaseLayout/Header.astro`): desktop nav links to `/projects`, `/blog`, and
  ...
  was fixed — it used to be hard-disabled (`display: none`) with no Blog/Contact links
...
- **Contact form flow**: `Contact/ContactForm.astro` (rendered on the index page inside
  ...
  Cloudflare Worker repo (`tobysmith568/email.tobysmith.uk`) — folded into this repo.**
  That old Worker is still deployed and still what production actually uses (GitHub Pages/its own
  Worker) until the production cutover; don't decommission it yet.
...
- **Styling**: plain component-scoped `<style>` blocks (native CSS nesting), no preprocessor —
  Sass was dropped (nothing here used a Sass feature without a native CSS equivalent
...
- **Package manager**: Bun (`bun.lock`). `astro`/`cypress` are
